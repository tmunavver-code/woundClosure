...
%% 2 - Output directory (today's date)
todayStr = datestr(now, 'yyyy-mm-dd');
saveDir = fullfile('results', ['force_closure_' todayStr]);
...
for tstep = 1:param.Nsteps
    ...
    %% Save a wound/tissue snapshot every 50 timesteps (50, 100, 150, ... 1000)
    if mod(tstep, 50) == 0
    ...
    if mod(tstep, 100) == 0
        fprintf('  tstep %d / %d, energy=%.4f, nT1=%d, wound area=%.2f%% of initial\n', ...
            tstep, param.Nsteps, energymat(tstep), param.nT1, 100*woundAreaLog(tstep)/celldata.woundArea0);
    end
end
...
fprintf('Final wound area: %.2f%% of initial\n', 100*woundAreaLog(end)/celldata.woundArea0);
